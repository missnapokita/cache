...
export function analyzeEpisodes(series, episodes) {
  ...
  // Always select the highest season that still has missing episode links.
  // A completely blank newer season is valid progress (EP0/total) and must
  // not fall back to an older partial Season 1.
  const ongoingSeason = progress.find((item) => item.isIncomplete);
