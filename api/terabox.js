...
// Cache reads are cheap and use the shared in-memory request limiter. Only a
// request that will actually call the upstream generator uses this additional
// Redis-backed per-IP limit, protecting API quota/cost across Vercel instances.
const GENERATION_RATE_WINDOW_SECONDS = positiveIntegerEnv(
  "TERABOX_GENERATION_RATE_WINDOW_SECONDS",
  60,
  10
);
const GENERATION_RATE_MAX = positiveIntegerEnv(
  "TERABOX_GENERATION_RATE_MAX",
  20,
  1
);
...
// When many viewers discover the same dead shared URL at nearly the same time,
// only the first recovery request may regenerate it. Later reports briefly
// reuse that replacement instead of starting a sequential refresh storm.
const RECOVERY_REFRESH_COOLDOWN_MS = 2 * 60 * 1000;
...
async function enforceGenerationRateLimit(req) {
  const clientHash = crypto
    .createHash("sha256")
    .update(getClientIp(req))
    .digest("hex")
    .slice(0, 24);
  const key = `${CACHE_NAMESPACE}:generation-rate:${clientHash}`;

  const count = Number(await redisCommand(["INCR", key]));

  /*
   * Refresh expiry on each attempt. This also repairs the very small chance
   * that a previous invocation stopped after INCR but before EXPIRE.
   */
  await redisCommand([
    "EXPIRE",
    key,
    String(GENERATION_RATE_WINDOW_SECONDS)
  ]);

  if (!Number.isFinite(count) || count <= GENERATION_RATE_MAX) {
    return;
  }

  const error = new Error("Too many stream generations");
  error.status = 429;
  error.retryAfter = GENERATION_RATE_WINDOW_SECONDS;
  throw error;
}
...
function createCacheRecord(data, recoveryRefresh = false) {
  ...
  if (recoveryRefresh) {
    record._bidamax_recovery_refresh_at = new Date(
      generatedAt
    ).toISOString();
  }
...
async function saveCache(
  ...
  const record = createCacheRecord(data, recoveryRefresh);
...
function isRecentRecoveryRefresh(record, now = Date.now()) {
  const refreshedAt = Date.parse(
    record?._bidamax_recovery_refresh_at || ""
  );

  return Number.isFinite(refreshedAt) &&
    now - refreshedAt >= 0 &&
    now - refreshedAt < RECOVERY_REFRESH_COOLDOWN_MS;
}
...
export default async function handler(req, res) {
  ...
  // Stops one client from flooding cache reads before Redis/upstream work.
  ...
  try {
    ...
    if (acquired) {
      try {
        let generationRateChecked = false;
        const ensureGenerationAllowed = async () => {
          if (generationRateChecked) return;
          await enforceGenerationRateLimit(req);
          generationRateChecked = true;
        };

        if (forceRefresh) {
          const cachedAfterLock = await readCache(cacheKey);

          if (isRecentRecoveryRefresh(cachedAfterLock, requestedAt)) {
            console.log(
              "[terabox-cache] FORCE_REFRESH_COOLDOWN_HIT",
              { cacheKey }
            );
            ...
            return json(
              ...
              200,
              {
                ...cachedAfterLock,
                cached: true,
                _bidamax_cache: "FORCE_REFRESH_COOLDOWN",
                _bidamax_ttl_seconds: CACHE_TTL_SECONDS
              },
              {
                "X-Bidamax-Cache": "FORCE_REFRESH_COOLDOWN"
              }
            );
          }

          await ensureGenerationAllowed();
    ...
    if (error?.status === 429) {
      ...
      return json(
        ...
        429,
        {
          error: "Too many stream generations",
          retryAfter
        },
