...
async function generateFromOrigin(sourceUrl, incomingAuthorization) {
  const originUrl = process.env.TERABOX_ORIGIN_URL || DEFAULT_ORIGIN;
  // Prefer a private Vercel variable. For immediate compatibility,
  // fall back to the Authorization header already sent by the current app.
  const originToken = process.env.TERABOX_ORIGIN_AUTH_TOKEN || incomingAuthorization;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      const generated = await generateFromOrigin(sourceUrl, req.headers.authorization);
